  ...
  if(any(plots %in% "Survey"))
  {
    ...
    if(length(strata.areas[,1]) > 0)
    {
      ...
      if(banks[i] %in% c("Sab","GBb")) 
      {
        ...
        # This will put the scale bottom middle I think...
        map.scale(min(smap.xlim)+0.1*(max(smap.xlim)-min(smap.xlim)),min(smap.ylim)+0.1*(max(smap.ylim)-min(smap.ylim)),relwidth = 0.15,cex=0.6,ratio=F)
    ...
    if(nrow(sb) > 0) # only run the rest of this if we have data...
    {
      ...
      for(j in 1:n.box)
      {
        ...
        ScallopMap(xlim = x.range,ylim = y.range,
                   ...
                   plot.bathy=T,plot.boundries = T,bathy.source="quick", xlab="",ylab="",
                   nafo.bord = F,nafo.lab = F,dec.deg = F,add.scale = add.scale)
